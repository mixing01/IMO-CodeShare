...
public class LargeNeighbourhoodSplit {
    static Random random = new Random();
    static int[] upOrDown = {-1, 1};
    public static Pair<TSPSolution, Integer> largeNeighbourhoodSplit(String path, double maxTime) throws FileNotFoundException {
        int iterNum = 0;
        TSPSolution solution = PointSplitter.splitFromFile(path);
        ...
        while (elapsedTime < maxTime) {
            ...
            repair(tempSolution);
            UpdatedSteep.steep(tempSolution);
    ...
    public static void destroy(TSPSolution solution, double destroyFrac, int spots) {
        ...
        int verticesDestroyed2 = ((int) Math.ceil(solution.cycle2.size()*tempFrac))/spots;
